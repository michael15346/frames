...
Решите задачи:
\begin{enumerate}
	\item{Пусть $\left\{f_k\right\}_{k=1}^m$ --- фрейм в пространстве $V$ с границами $A$, $B$; $P$ --- ортогональный проектор пространства $V$ на подпространство $W$. Докажите, что $\left\{Pf_k\right\}_{k=1}^m$ --- фрейм для $W$ с границами $A$, $B$.}
	\item{Пусть $\left\{e_1, e_2\right\}$ --- ортонормированный базис в $\mathbb{R}^2$. Для фрейма $\left\{e_1, \frac{1}{2} e_2, 2e_2\right\}$ найдите канонический двойственный (то есть фрейм $\left\{S^{-1} f_k\right\}$, где $\left\{f_k\right\}$ --- исходный фрейм с оператором $S$).}
	\item{Докажите, что $\left\{e^{2 \pi i k x}\right\}_{k \in \mathbb{Z}}$ --- фрейм в пространстве $L_2 \left[0, \frac{1}{2}\right]$. Каковы границы этого фрейма? Как действует оператор $S^{-1}$, если $S$ --- фреймовый оператор?}
\end{enumerate}
\subsection{Решение}
\subsubsection{Задача 1.1}
\begin{figure}[H]
\centering
\includegraphics[width=0.82\textwidth]{t1-1}
\end{figure}
Рассмотрим $w_A, w_B$ такие, что 
\[
	A \left| \left| w_A \right| \right|^2 = \sum_{k=1}^m \left| \left< w_A, f_k\right> \right|^2,
\]
\[
	\sum_{k=1}^m \left| \left< w_B, f_k\right> \right|^2 = B \left| \left| w_B\right| \right|^2
\]. Тогда для них также будет выполняться
\[
	\begin{aligned}
	A \left| \left| w_A \right| \right|^2 = \sum_{k=1}^m \left| \left< w_A, Pf_k\right> \right|^2 \\
	\sum_{k=1}^m \left| \left< w_B, Pf_k\right> \right|^2 = B \left| \left| w_B\right| \right|^2
\end{aligned},
\]
т.е. фрейм $\left\{Pf_h\right\}_{h=1}^m$ имеет границы $A, B$.
\subsubsection{Задача 1.2}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{t1-2}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{t1-3}
\end{figure}
\subsubsection{Задача 1.3}
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{t1-4}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{t1-5}
\end{figure}
\section{Задача 2}
Пусть $m=50,n=32,$
\[
